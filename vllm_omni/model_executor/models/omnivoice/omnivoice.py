...
_FULL_SEQUENCE_BUCKETS: tuple[int, ...] = (
    640, 768, 896, 1024, 1152, 1280, 1408, 1536, 1664,
)


def _full_sequence_bucket(required_len: int) -> int:
    if os.environ.get("VLLM_OMNI_OMNIVOICE_OPT") != "1":
        return int(required_len)
    for bucket in _FULL_SEQUENCE_BUCKETS:
        if required_len <= bucket:
            return int(bucket)
    return int(((required_len + 127) // 128) * 128)
...
class OmniVoiceMultiModalProcessor(BaseMultiModalProcessor[OmniVoiceMultiModalProcessingInfo]):
    ...
    def _call_hf_processor(
        ...
        text_tokens = self.text_tokenizer(text_prompt, return_tensors="pt").input_ids.squeeze(0)
        ...
        with torch.inference_mode():
            ref_audio_tokens = self.audio_tokenizer.encode(audio_signal)
            if ref_audio_tokens.dim() == 3:
                ref_audio_tokens = ref_audio_tokens.squeeze(0)
...
class OmniVoiceModel(
    ...
    def _forward_generator(self, input_ids: torch.Tensor, kwargs: dict) -> OmniOutput:
        ...
        if ref_audio_tokens is not None:
            ref_tokens = ref_audio_tokens.to(device)
        ...
        max_len = _full_sequence_bucket(max(cond_len, uncond_len))
        if cond_len < max_len:
            pad = torch.full(
        ...
        batch_input_ids = torch.stack([cond_ids, uncond_ids], dim=0)
